<script>
  ...
  $: strengthLabel = ["Tres faible", "Faible", "Moyen", "Bien", "Fort"][
    passwordScore
  ];
  $: strengthClass = ["very-weak", "weak", "medium", "good", "strong"][
    passwordScore
  ];
  ...
  function getErrorMsg(
    err,
    msg401 = "Verification echouee. Veuillez reessayer.",
  ) {
  ...
  async function handleUpdateEmail() {
    ...
    if (!emailsMatch)
      return showMessage("Les emails ne correspondent pas", "error");
    if (newEmail === user.email)
      return showMessage("Ce nouvel email est identique a l'actuel", "error");
    if (!currentPassword)
      return showMessage("Mot de passe requis pour modifier l'email", "error");
    try {
      savingEmail = true;
  ...
  async function handleUpdatePassword() {
    ...
    if (!passwordValid)
      return showMessage(
        "Le mot de passe ne respecte pas les regles requises",
        "error",
      );
    if (!passwordsMatch)
    ...
    try {
      ...
      showMessage(getErrorMsg(err), "error");
    } finally {
      savingPassword = false;
    }
  }
  ...
  async function handleDeleteAccount() {
    if (!deletePassword)
      return showMessage("Veuillez saisir votre mot de passe", "error");
          ...
          <form class="inline-form" on:submit|preventDefault={handleUpdateName}>
            ...
            <div class="inline-actions">
              <button type="submit" class="btn-validate" disabled={savingName}>
                {#if savingName}<span class="spinner-sm"
                  ></span>{:else}Valider{/if}
              </button>
              <button
                type="button"
                class="btn-cancel-soft"
                on:click={cancelEditName}>Annuler</button
              >
            </div>
          </form>
        {:else}
          <div class="value-row">
            ...
            <button
              ...
              aria-label="Modifier le nom">Modifier</button
            >
          </div>
        {/if}
      </div>
          ...
          >
            <div class="inline-fields">
              ...
              >
                ...
                {#if emailTouched && newEmail}
                  <span class="field-icon {emailValid ? 'ok' : 'ko'}"
                    >{emailValid ? "v" : "x"}</span
                  >
                {/if}
              ...
              <div
                class="field"
                class:has-error={confirmEmailTouched &&
                  confirmEmail &&
                  !emailsMatch}
              ...
              >
                ...
                {#if confirmEmailTouched && confirmEmail}
                  <span class="field-icon {emailsMatch ? 'ok' : 'ko'}"
                    >{emailsMatch ? "v" : "x"}</span
                  >
                {/if}
            ...
            <div class="inline-actions">
              <button type="submit" class="btn-validate" disabled={savingEmail}>
                {#if savingEmail}<span class="spinner-sm"
                  ></span>{:else}Valider{/if}
              </button>
              <button
                type="button"
                class="btn-cancel-soft"
                on:click={cancelEditEmail}>Annuler</button
              >
            </div>
          </form>
        {:else}
          <div class="value-row">
            ...
            <button
              class="btn-edit"
              on:click={startEditEmail}
              aria-label="Modifier l'email">Modifier</button
            >
          </div>
        {/if}
      </div>
      ...
      >
        <div class="field">
          <label for="currentPassword"
            >Mot de passe actuel <span class="required">*</span></label
          >
          <div class="input-wrapper">
        ...
        >
          <label for="newPassword"
            >Nouveau mot de passe <span class="required">*</span></label
          >
          <div class="input-wrapper">
            <input
              id="newPassword"
          ...
          {#if newPassword}
            ...
            <ul class="pwd-rules" id="pwd-rules-list" role="list">
              {#each [[passwordRules.length, "8 caracteres minimum"], [passwordRules.upper, "Une majuscule"], [passwordRules.lower, "Une minuscule"], [passwordRules.digit, "Un chiffre"]] as [ok, label]}
        ...
        <div
          class="field"
          class:has-error={confirmPasswordTouched &&
            confirmPassword &&
            !passwordsMatch}
        ...
        <button type="submit" disabled={savingPassword} class="btn-primary">
          {#if savingPassword}<span class="spinner-sm"></span> Enregistrement...{:else}Mettre
            a jour le mot de passe{/if}
        </button>
      </form>
    </div>

    <div
      class="section section-danger fade-in"
      style="animation-delay:0.25s"
      aria-labelledby="delete-title"
    >
      <h3 id="delete-title">Supprimer mon compte</h3>
      <p class="delete-warning">
        Cette action est <strong>irreversible</strong>. Toutes vos donnees
        seront definitvement supprimees.
      </p>
    ...
    <div class="modal">
      ...
      <div class="modal-body">
        <p class="modal-warning">
          Vous etes sur le point de <strong>supprimer definitivement</strong> votre
          compte et toutes vos donnees. Cette action ne peut pas etre annulee.
        </p>
        <form on:submit|preventDefault={handleDeleteAccount} novalidate>
          <div class="field">
            <label for="deletePassword"
              >Votre mot de passe <span class="required">*</span></label
            >
            <div class="input-wrapper">
              <input
                id="deletePassword"
          ...
          <div class="modal-actions">
            <button
              ...
              disabled={deletingAccount ||
                ...
                !deletePassword}
            >
              {#if deletingAccount}<span class="spinner-sm"></span> Suppression...{:else}Confirmer
                la suppression{/if}
            </button>
            <button
              type="button"
              class="btn-cancel-soft"
              on:click={closeDeleteModal}>Annuler</button
            >
          </div>
        </form>
      </div>
...
<style>
  ...
  .field.has-success input {
    ...
    box-shadow: 0 0 0 3px rgba(111, 207, 138, 0.12);
  }
